...
public class SalePlanServiceImpl implements SalePlanService {
    ...
    public void send() {
        ...
        logger.info("started");
        Session session = sessionService.createSession("SalePlan", DirectionEnum.EXPORT);
    ...
    private List<SessionMessage> saveMessages(Response response, Session session) {
        ...
        for (Response.Item item : response.getItem()) {
            ...
            sessionMessage.setObjectCode("SalePlan");
    ...
    private List<SalePlanInterface> updateStatuses(List<SalePlanInterface> list, List<SessionMessage> messages) {
        for (SalePlanInterface line: list) {
            SessionMessage message = messages.stream()
                .filter(t -> t.getObjectCode().equals("SalePlan"))
                .filter(t -> t.getObjectId().equals(line.getId().toString()))
